...
import com.feiyou.headstyle.util.Point2D;
...
public class MyHighlightView implements EditableDrawable.OnSizeChange {
    ...
    public void setScaleable(boolean scaleable) {
        ...
        if (scaleable) {
            mAnchorRotate = App.getApp().getResources().getDrawable(R.mipmap.aviary_resize_knob);
        } else {
            mAnchorRotate = null;
        }
    }
    ...
    public void setDeleteable(boolean deleteable) {
        if (deleteable) {
            mAnchorDelete = App.getApp().getResources().getDrawable(R.mipmap.ic_sticker_delete);
        } else {
            mAnchorDelete = null;
        }
    }
    ...
    public MyHighlightView(ImageView context, int styleId, FeatherDrawable content) {
        ...
        mAnchorRotate = App.getApp().getResources().getDrawable(R.mipmap.ic_sticker_control);
        mAnchorDelete = App.getApp().getResources().getDrawable(R.mipmap.ic_sticker_delete);
    ...
    public void draw(final Canvas canvas) {
        ...
        if (is_selected || is_focused) {
            ...
            if (mShowAnchors) {
                ...
                outlinePaint.setStrokeWidth(SizeUtils.dp2px(App.getApp(), 1));
    ...
    public int getHit(float x, float y) {
        ...
        if (mScaleEnabled) {
            ...
            if ((Math.abs(rect.left - x) < HIT_TOLERANCE) && verticalCheck
                    && AppUtils.checkBits(mResizeEdgeMode, GROW_LEFT_EDGE)) {
                Log.d(LOG_TAG, "left");
                retval |= GROW_LEFT_EDGE;
            }
            if ((Math.abs(rect.right - x) < HIT_TOLERANCE) && verticalCheck
                    && AppUtils.checkBits(mResizeEdgeMode, GROW_RIGHT_EDGE)) {
                Log.d(LOG_TAG, "right");
                retval |= GROW_RIGHT_EDGE;
            }
            if ((Math.abs(rect.top - y) < HIT_TOLERANCE) && horizCheck
                    && AppUtils.checkBits(mResizeEdgeMode, GROW_TOP_EDGE)) {
                Log.d(LOG_TAG, "top");
                retval |= GROW_TOP_EDGE;
            }
            if ((Math.abs(rect.bottom - y) < HIT_TOLERANCE) && horizCheck
                    && AppUtils.checkBits(mResizeEdgeMode, GROW_BOTTOM_EDGE)) {
    ...
    public void onMouseMove(int edge, MotionEvent event2, float dx, float dy) {
        ...
        if (edge == MOVE) {
            ...
            boolean is_left = AppUtils.checkBits(edge, GROW_LEFT_EDGE);
            boolean is_top = AppUtils.checkBits(edge, GROW_TOP_EDGE);
