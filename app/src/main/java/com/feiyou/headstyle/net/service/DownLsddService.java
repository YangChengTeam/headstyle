...
public class DownLsddService extends Service {
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (msg.what == MSG_UPDATE_PROGRESS && !hasStop) {
				...
				nm.notify(Constant.NOTIFICATION_LSDD_ID, notification);
	...
	private void createNotification() {
		...
		mBuilder.setContentTitle("开始下载铃声朵朵")
		.setTicker("铃声朵朵开始下载啦")
	...
	public void onDestroy() {
		...
		nm.cancel(Constant.NOTIFICATION_LSDD_ID);
		super.onDestroy();
	}
	...
	private class DownLoadThread extends Thread {
		...
		public void run() {
			...
			downloadFile = new DownloadFile(url, Constant.LSDD_SOURCE_FILE_PATH);

			File file = new File(Constant.LSDD_SOURCE_FILE_PATH);
			if (file.exists()) {
				file.delete();
			}
			...
			if (downloadFile.downloadFile()) {
				boolean result = FileUtils.renameFile(Constant.LSDD_SOURCE_FILE_PATH,Constant.LSDD_DOWN_FILE_PATH);
				if(result){
					Intent intent = new Intent(Intent.ACTION_VIEW);
					intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					intent.setDataAndType(Uri.fromFile(new File(Constant.LSDD_DOWN_FILE_PATH)),
